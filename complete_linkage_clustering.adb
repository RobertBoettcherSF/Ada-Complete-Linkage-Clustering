--  Complete_Linkage_Clustering body — Euclidean distances, max-linkage,
--  naive proximity-matrix agglomeration, dendrogram cut / height labels.

pragma Ada_2022;

with Ada.Numerics.Generic_Elementary_Functions;

package body Complete_Linkage_Clustering
  with SPARK_Mode => Off
is
   ...
   begin
      ...
      return Best;
   end Complete_Linkage_Distance;

   -------------------------------------------------------------------------
   -- Naive complete-linkage agglomeration
   -------------------------------------------------------------------------

   function Run_Complete_Linkage (Dist : Distance_Matrix) return Dendrogram is
      N : constant Natural := Dist'Length (1);
      ...
      --  Choose the pair with the *smallest* complete-linkage distance
      --  (most similar under max-link).  Wikipedia's naive step text
      --  incorrectly says "max d[(i),(j)]"; the working example correctly
      --  picks the minimum entry of the current matrix.
   ...
   begin
      ...
      if N < 2 then
         raise Invalid_Argument
           with "Run_Complete_Linkage needs at least 2 points";
      end if;
      ...
      return Tree;
   end Run_Complete_Linkage;

   function Run_Complete_Linkage (Data : Dataset) return Dendrogram is
   begin
      Require_Dataset (Data);
      ...
      return Run_Complete_Linkage (Build_Distance_Matrix (Data));
   end Run_Complete_Linkage;
